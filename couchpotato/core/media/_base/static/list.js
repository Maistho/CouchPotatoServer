var MediaList = new Class({
	...
	initialize: function(type, options){
		...
		self.el = new Element('div.' + type + 's').adopt(
			...
			self.media_list = new Element('div', {
				'events': {
					'click:relay(.media)': function(e, el){
						el.retrieve('klass').onClick(e);
					},
					'mouseenter:relay(.media)': function(e, el){
						(e).stopPropagation();
						el.retrieve('klass').onMouseenter(e);
					},
					'change:relay(.media input)': function(e, el){
						(e).stopPropagation();
						el = el.getParent('.media');
		...
		App.on(self.type + '.added', self.mediaAdded.bind(self));
		App.on(self.type + '.deleted', self.mediaDeleted.bind(self));
	},

	mediaDeleted: function(notification){
		var self = this;

		if(self.media_added[notification.data._id]){
			self.media.each(function(m){
				if(m.get('_id') == notification.data._id){
					m.destroy();
					delete self.media_added[notification.data._id];
	...
	mediaAdded: function(notification){
		var self = this;

		self.fireEvent(self.type + 'Added', notification);
		if(self.options.add_new && !self.media_added[notification.data._id] && notification.data.status == self.options.status){
			window.scroll(0,0);
			self.createMedia(notification.data, 'top');
			self.setCounter(self.counter_count+1);
	...
	addMedia: function(media, total){
		...
		// do scrollspy
		if(media.length < self.options.limit && self.scrollspy){
			self.load_more.hide();
			self.scrollspy.stop();
		}

		self.createMedia(media, 'bottom');
	...
	setCounter: function(count){
		...
		//TODO: add proper pluralization
		self.navigation_counter.set('text', count === 1 ? '1 ' + self.type : (count || 0) + self.type + 's');
		...
		if(self.total_media && count === 0 && !self.empty_message){
			...
			self.empty_message = new Element('.message', {
				//TODO: pluralization
				'html': 'No ' + self.type + 's found ' + message + '.<br/>'
			}).grab(
				new Element('a', {
					'text': 'Reset filter',
					'events': {
			...
			).inject(self.media_list);

		}

	},

	createMedia: function(m, inject_at, nr){
		var self = this,
			media = Array.isArray(m) ? m : [m],
			media_els = [];
		inject_at = inject_at || 'bottom';

		media.each(function(m, nr){

			var m = new window[self.type.capitalize()](self, {
			...
			if(inject_at === 'bottom'){
				...
				el.inject(self.media_list, inject_at);
			}

			self.media.include(m);
			self.media_added[m._id] = true;
		});

		if(media_els.length > 0){
			$(self.media_list).adopt(media_els);
		}
	...
	calculateSelected: function(){
		...
		var selected = 0,
			media = self.media.length;
		self.media.each(function(m){
			selected += m.isSelected() ? 1 : 0;
		});

		var indeterminate = selected > 0 && selected < media,
			checked = selected == media && selected > 0;
	...
	deleteSelected: function(){
		...
		var qObj = new Question('Are you sure you want to delete '+ids.length+' ' + self.type + (ids.length != 1 ? 's' : '') +'?', help_msg, [{
					...
					Api.request('media.delete', {
						...
						'onSuccess': function(){
							...
							self.media.each(function(m){
								if (m.isSelected()){
									$(m).destroy();
									erase_media.include(m);
								}
							});

							erase_media.each(function(m){
								self.media.erase(m);
								m.destroy();
								self.setCounter(self.counter_count-1);
								self.total_media--;
	...
	changeQualitySelected: function(){
		var self = this;
		var ids = self.getSelectedMedia();

		Api.request(self.type + '.edit', {
			'method': 'post',
			'data': {
				'id': ids.join(','),
	...
	getMedia: function(reset){
		...
		var loader_timeout;
		if(self.media.length === 0 && self.options.loader){
		...
		Api.request(self.options.api_call, {
			'data': Object.merge({
				'type': self.options.type || self.type,
				'status': self.options.status,
			...
			'onSuccess': function(json){
				...
				var json_media = json[self.type + 's'];
				self.store(json_media);
				self.addMedia(json_media, json.total || json_media.length);
				if(self.scrollspy) {
					self.load_more.set('text', 'load more ' + self.type);
					self.scrollspy.start();
				}
	...
	store: function(media){
		var self = this;

		self.offset += media.length;
	...
	checkIfEmpty: function(){
		...
		var is_empty = self.media.length === 0 && (self.total_media === 0 || self.total_media === undefined);
		...
		if(is_empty && self.options.on_empty_element){
			...
			ee.inject(self.loader_first || self.title || self.media_list, 'after');
